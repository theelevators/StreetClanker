...
import { copyTrs, mat4Identity, mat4Invert, mat4Multiply, mat4ToTrs, trsToMat4, } from "./math_trs.js";
...
function writeGlobal(world, entity, next) {
    if (!world.has(entity, GlobalTransform)) {
        world.add(entity, GlobalTransform({ ...next }));
        return;
    }
    const global = world.get(entity, GlobalTransform);
    if (trsEqual(global, next))
        return;
    copyTrs(next, global);
    world.markChanged(entity, GlobalTransform);
}
function propagateEntity(world, entity) {
...
export function recomputeLocalPreservingGlobal(world, child, newParent) {
    const global = world.get(child, GlobalTransform) ?? world.get(child, Transform);
    ...
    const pg = world.get(newParent, GlobalTransform) ?? world.get(newParent, Transform);
    if (!pg) {
        copyTrs(global, local);
        world.markChanged(child, Transform);
        return;
...
//# sourceMappingURL=transform_propagate.js.map