...
import { WasmAbiExecutor, } from "./wasm_executor.js";
import { sharedWasmMemoryAvailable, webAssemblyAvailable, } from "../storage/wasm_memory.js";
import { runAbiSystemLocalSync } from "./abi_system.js";
const WASM_META = Symbol.for("mob3.wasmSystemMeta");
function assertWasmAccess(access, name) {
    assertWorkerAccess(access, name);
    for (const c of [...access.componentRead, ...access.componentWrite]) {
        if (c.isTag)
            continue;
        if (!isPackedComponent(c)) {
            throw new Error(`wasmSystem '${name}': requires packed components (got '${getPackedMeta(c)?.name ?? c.name ?? "?"}')`);
        }
        const meta = getPackedMeta(c);
        if (!meta.shared || meta.backing !== "wasm") {
            throw new Error(`wasmSystem '${name}': component '${meta.name}' must use shared: true, backing: "wasm"`);
        }
    }
}
function getArenaOrThrow(world, name) {
    const arena = world.getWasmArena();
    if (!arena) {
        throw new Error(`wasmSystem '${name}': World has no WasmMemoryArena (App({ wasmArena: true }) or world.setWasmArena)`);
    }
    return arena;
}
export function ensureWasmExecutor(meta, world) {
    if (meta.executor)
...
function runFallback(world, meta) {
    if (!meta.fallback) {
        throw new Error(`wasmSystem '${meta.name}': WASM unavailable and no JS fallback provided`);
    }
    const fake = {
        id: meta.id,
        name: meta.name,
...
function wasmAvailable(world) {
    return (webAssemblyAvailable() &&
        sharedWasmMemoryAvailable() &&
        !!world.getWasmArena()?.shared);
}
/** Warm compile/instantiate before sync ticks. */
export async function warmWasmSystem(world, meta) {
    if (!wasmAvailable(world))
        return;
    const ex = ensureWasmExecutor(meta, world);
    await ex.ensureReady();
}
export async function runWasmSystem(world, meta) {
    if (!wasmAvailable(world)) {
        if (meta.mode === "required") {
            throw new Error(`wasmSystem '${meta.name}': WASM shared memory required but unavailable`);
        }
...
export function runWasmSystemSync(world, meta) {
    if (!wasmAvailable(world)) {
        if (meta.mode === "required") {
            throw new Error(`wasmSystem '${meta.name}': WASM shared memory required but unavailable`);
        }
    ...
    if (!executor.ready) {
        ...
        throw new Error(`wasmSystem '${meta.name}': module not ready — await warmWasmSystem() or updateAsync first`);
    }
 ...
 */
export function wasmSystem(def) {
    if (def.access.commands) {
        throw new Error(`wasmSystem '${def.name}': commands are outside ABI v1 (host-side only)`);
    }
...
export function getWasmMeta(fn) {
    return fn[WASM_META];
}
export function isWasmSystem(fn) {
    return WASM_META in fn;
}
export { WASM_META };
//# sourceMappingURL=wasm_system.js.map