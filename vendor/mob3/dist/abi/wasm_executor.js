...
export class WasmAbiExecutor {
    ...
    async ensureReady() {
        if (this.disposed)
            throw new Error("WasmAbiExecutor disposed");
        if (this.instance)
            return;
    ...
    instantiateFromBytes(bytes) {
        ...
        if (typeof abiFn === "function") {
            this.abiVersionFn = abiFn;
            const v = this.abiVersionFn() | 0;
            if (v !== ABI_VERSION) {
                throw new AbiError("unsupported_version", `WASM system "${this.systemName}" expects mob3 Execution ABI v${v}. Runtime provides v${ABI_VERSION}.`, this.systemName);
            }
        }
        const run = exports[this.exportName];
        if (typeof run !== "function") {
            throw new Error(`WASM system "${this.systemName}": missing export '${this.exportName}' in module`);
        }
        this.runFn = run;
    }
    ...
    executeSync(invocation) {
        ...
        try {
            ...
            if (!this.instance || !this.runFn) {
                throw new Error(`WasmAbiExecutor "${this.systemName}" not ready — call ensureReady() first`);
            }
            ...
            if (!offsets) {
                throw new Error(`WASM system "${this.systemName}": integrate binding requires Transform+Velocity f32 xyz`);
            }
            this.runFn(n, delta, offsets.tx, offsets.ty, offsets.tz, offsets.vx, offsets.vy, offsets.vz);
            return {
                abiVersion: ABI_VERSION,
                systemId: invocation.system.id,
                status: "ok",
                execMs: (typeof performance !== "undefined" ? performance.now() : Date.now()) -
                    t0,
            };
        }
        catch (err) {
    ...
    bindInvocation(invocation) {
        ...
        for (const exp of this.expects) {
            ...
            if (!store) {
                throw new AbiError("missing_store", `WASM system "${this.systemName}" requires store '${exp.name}', but invocation did not provide it`, this.systemName);
            }
            const fields = new Map();
            for (const fExp of exp.fields) {
                const field = store.fields.find((f) => f.name === fExp.name);
                if (!field) {
                    throw new AbiError("missing_field", `WASM system "${this.systemName}" expected field '${fExp.name}' on '${exp.name}'`, this.systemName);
                }
                if (field.type !== fExp.type) {
                    throw new AbiError("type_mismatch", `WASM system "${this.systemName}" store '${exp.name}.${fExp.name}': expected ${fExp.type}, got ${field.type}`, this.systemName);
                }
                if (field.buffer !== arenaBuf) {
                    throw new Error(`WASM system "${this.systemName}": field '${exp.name}.${fExp.name}' buffer is not the arena WebAssembly.Memory — zero-copy shared path required`);
                }
