import { Name, Transform, GlobalTransform, } from "mob3";
...
import { bindMixerEvents, getEntityGeneration, indexBones, stripRootMotion, } from "./runtime.js";
import { animationStoreOf } from "./systems.js";
function cloneAnimatedRoot(scene) {
    let skinned = false;
    scene.traverse((o) => {
        if (o.isSkinnedMesh)
            skinned = true;
 ...
 */
export function instantiateAnimatedGltf(world, handle, opts = {}) {
    const assets = assetsOf(world);
    const state = assets.state(handle);
    if (state.status !== "ready") {
        throw new Error(`Cannot instantiate animated GltfAsset "${handle.key}": asset state is ${state.status}.`);
    }
    if (!state.value.clips.length) {
        throw new Error(`Cannot instantiate animated GltfAsset "${handle.key}": asset has no AnimationClips.`);
    }
    ...
    if (sceneRes)
        sceneRes.add(cloned);
    const entity = world.spawn(Name({ value: cloned.name }), Transform(), GlobalTransform(), ThreeObject(cloned), AnimationPlayer({
        clip: opts.clip ?? "",
        playing: !!opts.clip,
        loop: opts.loop ?? true,
        speed: 1,
    ...
    if (opts.parent != null)
        world.setParent(entity, opts.parent);
...
const animatedReleases = new Map();
function registerAnimatedRelease(entity, fn) {
    animatedReleases.set(entity, fn);
}
export function despawnAnimatedGltf(world, root) {
 ...
 */
export function createProceduralCharacterAsset() {
    ...
    const rootMotion = new THREE.VectorKeyframeTrack("Character.position", [0, 1], [0, 0, 0, 5, 0, 0]);
...
function makeClip(name, bone, amp, _duration, period) {
    const times = [0, period / 2, period];
    const values = [0, amp, 0];
    const track = new THREE.NumberKeyframeTrack(`${bone}.rotation[z]`, times, values);
    return new THREE.AnimationClip(name, period, [track]);
}
//# sourceMappingURL=instantiate_animated.js.map